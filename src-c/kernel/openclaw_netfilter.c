/*
 * openclaw_netfilter.c — Netfilter hook for network-aware AI
 *
 * Hooks into the LOCAL_OUT chain to log/inspect outbound connections.
 * Events are made available to userspace via /proc/openclaw/netfilter
 ...
 */
static unsigned int openclaw_nf_hook(void *priv,
                                      struct sk_buff *skb,
                                      const struct nf_hook_state *state)
{
    ...
    if (iph->protocol == IPPROTO_TCP) {
        ...
        if (tcph) {
            ...
            if (tcph->syn && !tcph->ack && openclaw_get_log_level() >= 2) {
                pr_info("openclaw: TCP SYN %pI4:%d -> %pI4:%d\n",
...
int openclaw_nf_get_recent(char *buf, size_t buf_size)
{
    unsigned long flags;
    int pos = 0;
    int i, idx;
...
int openclaw_netfilter_init(void)
{
    memset(nf_log, 0, sizeof(nf_log));
    nf_log_pos = 0;

    nf_hook_ops.hook     = openclaw_nf_hook;
    ...
    if (ret < 0) {
        pr_err("openclaw: failed to register netfilter hook: %d\n", ret);
        return ret;
    }

    pr_info("openclaw: netfilter hook registered (LOCAL_OUT)\n");
    return 0;
}

void openclaw_netfilter_exit(void)
{
    nf_unregister_net_hook(&init_net, &nf_hook_ops);
    pr_info("openclaw: netfilter hook unregistered\n");
}
