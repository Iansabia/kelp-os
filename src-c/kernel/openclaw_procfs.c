/*
 * openclaw_procfs.c — /proc/openclaw runtime stats
 *
 * Provides human-readable and machine-parseable statistics
 * about the OpenClaw kernel module.
 */
...
extern int openclaw_nf_get_recent(char *buf, size_t buf_size);
...
/*
 * /proc/openclaw/stats — show module statistics
 */
static int stats_show(struct seq_file *m, void *v)
{
    struct oclaw_stats *st = openclaw_get_stats();
    ktime_t now = ktime_get_boottime();
    s64 uptime_ns = ktime_to_ns(ktime_sub(now, openclaw_get_start_time()));
...
/*
 * /proc/openclaw/netfilter — show recent network events
 */
static int netfilter_show(struct seq_file *m, void *v)
{
    ...
    if (!openclaw_get_netfilter_enabled()) {
        seq_printf(m, "  (netfilter hooks disabled)\n");
        return 0;
    }

    len = openclaw_nf_get_recent(buf, sizeof(buf));
    if (len > 0) {
        seq_printf(m, "%s", buf);
    } else {
...
int openclaw_procfs_init(void)
{
    /* Create /proc/openclaw directory */
    proc_dir = proc_mkdir(OPENCLAW_PROC_NAME, NULL);
    if (!proc_dir) {
        pr_err("openclaw: failed to create /proc/%s\n", OPENCLAW_PROC_NAME);
        return -ENOMEM;
    }

    /* Create /proc/openclaw/stats */
    proc_stats = proc_create("stats", 0444, proc_dir, &stats_proc_ops);
    if (!proc_stats) {
        pr_err("openclaw: failed to create /proc/%s/stats\n", OPENCLAW_PROC_NAME);
        proc_remove(proc_dir);
        return -ENOMEM;
    }

    /* Create /proc/openclaw/netfilter */
    proc_netfilter = proc_create("netfilter", 0444, proc_dir, &netfilter_proc_ops);
    if (!proc_netfilter) {
        pr_err("openclaw: failed to create /proc/%s/netfilter\n", OPENCLAW_PROC_NAME);
        proc_remove(proc_stats);
        proc_remove(proc_dir);
        return -ENOMEM;
    }

    pr_info("openclaw: procfs entries created under /proc/%s\n", OPENCLAW_PROC_NAME);
    return 0;
}

void openclaw_procfs_exit(void)
{
    proc_remove(proc_netfilter);
    proc_remove(proc_stats);
    proc_remove(proc_dir);
    pr_info("openclaw: procfs entries removed\n");
}
